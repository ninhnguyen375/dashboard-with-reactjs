...
export const createProduct = product => {
  return async (dispatch, getState) => {
    try {
      ...
      return dispatch({ type: "CREATE_ERROR", err });
    }
  };
};
  ...
  return async (dispatch, getState) => {
    try {
      ...
      products.forEach(product => {
        del.push(Axios.delete(`http://localhost:3001/api/products/${product}`));
      });
      await Promise.all(del);
      dispatch(getProductsWithRedux());
      return dispatch({ type: "DELETE_SUCCESS", numDeleted: products.length });
    } catch (err) {
      return dispatch({ type: "DELETE_ERROR", err });
    }
  };
};
...
export const getProductsAndCategories = () => {
  return async dispatch => {
    try {
      ...
      const producers = await Axios("http://localhost:3001/api/producers/");
      return dispatch({
